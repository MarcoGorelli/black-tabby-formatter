...
def test_main(tmpdir):
    ...
    with open(file, 'w') as fd:
        fd.write(content)
    output = subprocess.run(['python', '-m', 'black_tabby_formatter', file], universal_newlines=True, capture_output=True)
...
def test_main_multiple_files(tmpdir):
    ...
    with open(file2, 'w') as fd:
        fd.write(content2)
    output = subprocess.run(['python', '-m', 'black_tabby_formatter', file1, file2], universal_newlines=True, capture_output=True)
